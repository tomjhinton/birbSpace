...
void uvRipple(inout vec2 uv, float intensity){
    ...
    float cLength=length(p);
     ...
     uv= uv +(p/cLength)*cos(cLength*15.0-vTime*.5)*intensity;
...
void coswarp2Minus(inout vec2 trip, float warpsScale ){
...
float plot(vec2 st, float pct){
  return  smoothstep( pct-st.x, pct, st.y) -
          smoothstep( pct +st.y, pct, st.y);
}

vec3 shape( in vec2 p, in float sides )
{
...
void main(){
  ...
  vec2 uv3 = vUv;
  vec2 uv4 = vUv;
  vec2 uv5 = getRadialUv(vUv - .5);
      vec2 uv6 = vUv;
  // uv +=.5;

    // uvRipple(uv, .05);
    vec2 roteA = rotateUV(uv, vec2(.5), PI * vTime * .01);
    vec2 roteC = rotateUV(uv, vec2(.5), -PI * vTime * .05);


  uv5 = modPolar(vec2(uv5.x, uv.y) -.5  , 9. );

    // uv = modPolar(vec2(uv) -.5   , 9. );
    // // uv = tile(uv, 12. );
    // uv =rotateTilePattern(uv);



    pMod2(uv2, vec2(9. * sin(vTime  * .05), 5. * sin(vTime * .04)));

    uv2 = modPolar(vec2(roteA) -.4   , 5. );
    // uvRipple(uv, .5);
    uv2 = tile(uv2, 5. );
  ...
  float triangle1 = stroke(triangleDF(uv), .7, .95);
  float triangle2 = stroke(triangleDF(uv5), .6, .95);
  float triangle3 = stroke(triangleDF(uv), .5, .95);
  float triangle4 = stroke(triangleDF(uv), .4, .5);


    // uv = rotateTilePattern(vec2(uv.x , uv.y));

    vec3 color = vec3(1.);

    vec3 color2 = vec3(triangle2, stroke(uv.y, .5, .5), stroke(uv2.x, .5, .5));

      vec3 color3 = vec3( stroke(uv.y, .5, .5), triangle4, stroke(uv.x, .5, .5));


      color.r = stroke(cnoise(uv * 4. * uv2.x),.5,.5);

      color.g = stroke(cnoise(uv * 4. * uv2.y),.5,.6);

        color.b = stroke(cnoise(uv * 6. + uv2.y),.5,.6);


        coswarp2Minus(color.rg, 1.5);

        coswarp2(color.rg, 2.);
          color3.gb = modPolar(color.gb, 3.);




    // color = mix(color, color2, triangle3);
    // color = mix(color, color3, triangle4);
    //
    //    coswarp(color, 3.);

    color = vec3(stroke(cnoise(roteA * 20. * cnoise(roteC)),.5,.5));
